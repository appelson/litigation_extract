# =============================================================================
# 01_load_clean.R
# Loads raw case + complaint files, cleans them, and saves to data/
# =============================================================================
...
filtered_cases <- all_cases %>%
  ...
  filter(
    !is.na(terminated),
    filed_on   < DATE_CUTOFF,
    terminated < DATE_CUTOFF
  ) %>%
...
# Add per-case document ordering for PDFs
pdf_order <- downloads %>%
...
message(glue::glue(
  "Saved: {nrow(filtered_cases)} cases | ",
  "{nrow(filtered_texts)} texts | ",
  "{nrow(pdf_documents)} PDFs"
))
